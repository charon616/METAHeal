...
float vertexNoise(float3 p, float t) {
  float n1 = sin(dot(p, float3(12.9898, 78.233, 37.719)) + t);
  float n2 = sin(dot(p, float3(39.3467, 11.135, 83.155)) - t * 1.37);
  float n3 = sin(dot(p, float3(73.156, 52.235, 9.151)) + t * 2.11);
...
VertexOutput baseVertex(VertexInput v) {
  ...
  float4 animatedVertex = v.vertex;
  float waveOffset = sin(_Time.y * _WaveSpeed + v.vertex.y * _WaveFreq) * _WaveAmp;
  float noiseOffset = vertexNoise(v.vertex.xyz * _NoiseScale, _Time.y * _NoiseSpeed) * _NoiseAmp;
  animatedVertex.xyz += v.normal * (waveOffset + noiseOffset);

  o.worldPos = mul(unity_ObjectToWorld, animatedVertex);
...
half4 baseFragment(VertexOutput i) : SV_Target {
  ...
  // wave
  float wave = sin(i.worldPos.y * _WaveFreq + _Time.y * _WaveSpeed);

  // noise (reuse existing mask)
  float noise = tex2D(_FingerGlowMask, i.texcoord1.xy * 2 + _Time.y * 0.2).r;

  // mix
  float flow = lerp(wave * 0.5 + 0.5, noise, 0.5);

  // smooth
  float glow = smoothstep(0.4, 0.7, flow);

  // fresnel boost
  float fresBoost = pow(1.0 - saturate(dot(i.worldNormal, worldViewDir)), _FresnelPower);
  float colorPulse = wave * 0.5 + 0.5;

  // add
  color.rgb += _CyanColor.rgb * glow * fresBoost * _GlowStrength;
  color.rgb = lerp(color.rgb, _CyanColor.rgb, colorPulse * (_GlowStrength * 0.25));
